...
namespace TitleGame
{
    public class LivesManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            } else if (Lives < Contans.maxLivesCount)
            {
        ...
        private IEnumerator InfiniteLivesCoroutine()
        {
            ...
            SetLifes(Contans.maxLivesCount);
        ...
        private IEnumerator LivesCoroutine()
        {
            ...
            var wait = new WaitForSeconds(0.25f);
            while (Lives < Contans.maxLivesCount)
            {
        ...
        private class LivesSave : ISaveObject
        {
            ...
            public void Init(LivesData data)
            {
                if (firstTime)
                {
                    ...
                    livesCount = Contans.maxLivesCount;
                    date = DateTime.Now;
                }
                else
                {
                    ...
                    if (livesCount < Contans.maxLivesCount)
                    {
                        var timeDif = DateTime.Now - date;

                        var oneLifeSpan = TimeSpan.FromSeconds(data.oneLifeRestorationDuration);

                        while (timeDif >= oneLifeSpan && livesCount < Contans.maxLivesCount)
                        {
                            timeDif -= oneLifeSpan;
                            date += oneLifeSpan;
